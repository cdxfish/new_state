...
class GroundTraffic(models.Model):
    _name = 'funenc_xa_station2.ground_traffic'
    _description = u'地面交通'
    _rec_name = 'ground_environment_ids'
    _inherit = 'fuenc_station.station_base'

    ground_environment_ids = fields.One2many('funenc_xa_station2.ground_environment','ground_traffic_id',string='地面环境')
    bus_lines =  fields.One2many('funenc_xa_station2.bus_line','ground_traffic_id',string='交通线路表')


class BusLine(models.Model):
    _name = 'funenc_xa_station2.bus_line'
    ...
    ground_traffic_id = fields.Many2one('funenc_xa_station2.ground_traffic',string='地面交通关联')


class GroundEnvironment(models.Model):
    _name = 'funenc_xa_station2.ground_environment'
    _description = u'地面环境'
    _rec_name = 'entrance_exit'
