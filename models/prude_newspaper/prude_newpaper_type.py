...
class PrudeNewpaper(models.Model):
    _name = 'funenc_xa_station2.prude_newpaper_type'
    ...
    def prude_newpaper_type_selete(self):
        self.env['funenc_xa_station2.prude_newpaper_type'].search([('id','=',self.id)]).unlink()
    ...
    def prude_newpaper_type_onchange(self):
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'funenc_xa_station2.prude_newpaper_type',
    ...
    def fixed_model(self, domain):
        ...
        for i in value:
            if not self.env['funenc_xa_station2.prude_newpaper_type'].search_read([('prude_event_type','=',i['prude_event_type'])]):
                self.env['funenc_xa_station2.prude_newpaper_type'].sudo().create(i)
