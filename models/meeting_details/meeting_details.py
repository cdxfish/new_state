...
class MeetingDateils(models.Model):
    _name = 'funenc_xa_station2.meeting_dateils'
    ...
    shifts_id = fields.Many2one('funenc_xa_station2.production_change_shifts', string='交接班')

    # 创建一条新的记录
    def new_increase_record(self):
        view_form = self.env.ref('funenc_xa_station2.meeting_details_form').id
        return {
            'name': '会议记录',
            'type': 'ir.actions.act_window',
            "views": [[view_form, "form"]],
            'res_model': 'funenc_xa_station2.meeting_dateils',
            'target': 'new',
        }
    ...
    def meet_dateils_button(self):
        view_form = self.env.ref('funenc_xa_station2.meeting_details_details').id
        return {
            'name': '证件名称',
            'type': 'ir.actions.act_window',
            "views": [[view_form, "form"]],
    ...
    def meet_delete_button(self):
        self.env['funenc_xa_station2.meeting_dateils'].search([('id','=',self.id)]).unlink()
