...
class delivery_storage(models.Model):
    _name = 'funenc_xa_station2.delivery_storage'
    _description = u'耗材出库'
    ...
    store_house_ids = fields.One2many('funenc_xa_station2.delivery_storage_to_consumables_inventory','delivery_storage_id',string='出库仓库')
    is_delivery = fields.Selection(selection=[('yes','已出库'),('no','未出库')],default="no")
    consumables_apply_id = fields.Many2one('funenc_xa_station2.consumables_apply',string='申请出库关联')

    @api.multi
    def get_day_plan_publish_action(self):
        view_tree = self.env.ref('funenc_xa_station2.funenc_xa_station_delivery_storage_list').id
        return {
            'name': '耗材入库',
            'type': 'ir.actions.act_window',
            # 'domain': domain,
            "views": [[view_tree, "tree"]],
            'res_model': 'funenc_xa_station2.delivery_storage',
            ...
            "top_widget_options": '''{'tabs':
                                          [
                                              {
                                                  'title': '耗材入库',
                                                  'action' : 'funenc_xa_station2.xa_station_consumables_delivery_storage',
    ...
    def delivery_storage_save(self):
        ...
        if self.consumables_count == sel_inventory_count:
            ...
            self.env['funenc_xa_station2.consumables_warehousing'].create(values)
...
class delivery_storage_to_consumables_inventory(models.Model):
    ...
    _name = 'funenc_xa_station2.delivery_storage_to_consumables_inventory'
    delivery_storage_id = fields.Many2one('funenc_xa_station2.delivery_storage',string='出库关联')
    consumables_inventory_id = fields.Many2one('funenc_xa_station2.consumables_inventory',string='库存关联')
    inventory_count = fields.Integer(string='库存数量', related='consumables_inventory_id.inventory_count')
    consumables_type = fields.Many2one('funenc_xa_station2.consumables_type', string='耗材类型',related='consumables_inventory_id.consumables_type')
