...
class AwardRecord(models.Model):
    _name = 'funenc_xa_station2.award_record'
    ...
    award_project = fields.Many2one('funenc_xa_station2.award_standard',string='奖励项目')
    ...
    award_record_add = fields.One2many('funenc_xa_station2.award_record_add','associated',string='新增责任人员')
    ...
    def get_action(self, domain):
        view_tree = self.env.ref('funenc_xa_station2.award_record_tree').id
        return {
            'name': '奖励记录',
    ...
    def check_record_delete(self):
        self.env['funenc_xa_station2.award_record'].search([('id', '=', self.id)]).unlink()

    #修改当前的一条记录
    def check_record_change(self):
        view_form = self.env.ref('funenc_xa_station2.award_record_form_modify').id
        return {
            'name': '奖励记录',
            'type': 'ir.actions.act_window',
...
class AwardRecordAdd(models.Model):
    _name = 'funenc_xa_station2.award_record_add'

    staff = fields.Many2one('cdtct_dingtalk.cdtct_dingtalk_users',string='员工')
    jobnumber = fields.Char(related='staff.jobnumber',string='工号',readonly=True)
    award_money = fields.Char(string='奖励金额')
    incident_describe = fields.Char(string='事件描述')
    associated = fields.Many2one('funenc_xa_station2.award_record')
