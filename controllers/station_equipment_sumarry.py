...
class BelongManagemnet(http.Controller):
    @http.route('/funenc_xa_station/station_equipment_summary', auth='public', csrf=False, type='http')
    def import_excel_belong_to_management(self, **kw):
        ...
        # 打开模板excel文件进行读写操作
        rdbook = xlrd.open_workbook(path + 'station_equipment_summary.xls')
        # 复制模板
        ...
        if len(arr_data) > 0:
            for record in arr_data:
                worksheet.write(row, 0, record['equipment_fname'])
                worksheet.write(row, 1, record['equipment_count'])
                row += 1
