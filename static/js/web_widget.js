 ...
 */
odoo.define("web_widget", function(require) {
  "use strict";
  ...
  var web_widget = AbstractField.extend({
      ...
      if (mode == "edit") {
        ...
        self
          ._rpc({
            model: model,
            method: "search_read",
            domain: [['belong_management_imange', "=", self.res_id]]
          })
          .then(function(data) {
            if (data) {
            console.log(data);
              var $el = $(
                '<div class="web_widget" style="display: flex; flex-wrap: wrap; justify-content: flex-start"></div>'
              );
              for (var i in data) {
  ...
  registry.add("web_widget", web_widget);
  return { web_widget: web_widget };
});
